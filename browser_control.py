...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".zyon_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".zyon_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ZYON profile")
                zyon = str(Path.home() / ".zyon_profiles" / "firefox_zyon")
                Path(zyon).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(zyon, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".zyon_profiles" / "safari")
        ...
        zyon_profile = str(Path.home() / ".zyon_profiles" / self.browser_name)
        Path(zyon_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with ZYON profile: {zyon_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(zyon_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with ZYON profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "zyon_screenshot.png")
